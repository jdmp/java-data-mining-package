    ...
    try {
      ...
      for (int t = 0; t < Math.min(10, plotSettings.getMatrixGUIObject().getColumnCount()); t++) {
          ...
          if (plotSettings.isShowTrace(t)) {
              ...
              double sum=plotSettings.getMatrixGUIObject().getDoubleValueAt((long)plotSettings.getMinXValue(), column);
              double average = sum;
              ...
              for (double xr = plotSettings.getMinXValue() + xs; xr <= plotSettings.getMaxXValue(); xr += xs) {
                  ...
                  sum += yv2;
                  nmbOfPoints++;
                  if(nmbOfPoints>((double)plotSettings.getRunningAverageLength())/xs){
                    sum=sum-yvRA;
                    firstPoint += xs;
                    nmbOfPoints--;
                  }
