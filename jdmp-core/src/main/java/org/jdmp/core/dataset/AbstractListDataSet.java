...
public abstract class AbstractListDataSet extends AbstractListMatrix<Sample> implements ListDataSet {
    ...
    public void normalizeInput() {
        Matrix min = getInputMatrix().min(Ret.NEW, Matrix.ROW);
        Matrix max = getInputMatrix().max(Ret.NEW, Matrix.ROW);
        Matrix diff = max.minus(min);
        Matrix zeros = Matrix.Factory.zeros(diff.getSize());
        for (Sample s : this) {
            Matrix input = s.getAsMatrix(Sample.INPUT);
            input.minus(Ret.ORIG, true, min);
            input.divide(Ret.ORIG, true, diff);
            input.times(Ret.ORIG, true, 2.0);
            input.minus(Ret.ORIG, true, 1.0);
            input.replaceMissingBy(zeros);
            input.fireValueChanged();
        }
    }
