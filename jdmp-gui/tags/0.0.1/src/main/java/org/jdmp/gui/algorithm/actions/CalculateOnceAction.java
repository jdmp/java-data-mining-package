...
public class CalculateOnceAction extends AlgorithmAction {
	...
	public CalculateOnceAction(JComponent c, Algorithm a, Matrix m) {
		super(c, a);
		this.matrix = m;
	...
	public CalculateOnceAction(JComponent c, Algorithm a) {
		super(c, a);
	...
	public Object call() throws Exception {
		if (matrix == null) {
			getAlgorithm().calculate();
		} else {
			List<Matrix> ml = getAlgorithm().calculate(matrix);
			if (ml != null) {
				for (Matrix m : ml) {
					// m.showFrame();
				}
